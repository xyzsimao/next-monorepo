...
import { getProjectTailwindVersionFromConfig1 } from "@/src/utils/get-project-info"
...
async function addProjectComponents(
  ...
  const tailwindVersion = await getProjectTailwindVersionFromConfig1()
  ...
  // if (!options.skipFonts) {
  //   await updateFonts(tree.fonts as RegistryFontItem[] , config, {
  //     silent: options.silent,
  //   })
  // }
...
async function addWorkspaceComponents(
  ...
  const tailwindVersion =
    await getProjectTailwindVersionFromConfig1()
